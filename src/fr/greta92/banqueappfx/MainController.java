...
//	ListView<String> compteListe;
	/**
	 * JavaFX injecte l'objet de type ListView<Compte> dans la variable
	 * 'compteListe' l'élément ListView déclaré dans 'Main.java' doit posséder
	 * l'identifiant 'compteListe' : fx:id="compteListe"
	 */
	@FXML
	ListView<Compte> compteListe;
	/**
	 * JavaFX injecte l'objet de type SplitPane dans la variable 'splitPane'
	 * l'élément SplitPane déclaré dans 'Main.java' doit posséder l'identifiant
	 ...
	 * l'élément AnchorPane déclaré dans 'Main.java' doit posséder l'identifiant
	 * 'leftPane' : fx:id="leftPane"
	 */
	@FXML
	AnchorPane leftPane;
	/**
	 * JavaFX injecte l'objet de type TextField dans la variable 'numeroCompteTF'
	 * l'élément TextField déclaré dans 'Main.java' doit posséder l'identifiant
	 * 'numeroCompteTF' : fx:id="numeroCompteTF"
	 */
	@FXML
	 ...
	 * l'élément TextField déclaré dans 'Main.java' doit posséder l'identifiant
	 * 'titulaireTF' : fx:id="titulaireTF"
	 */
	@FXML
	TextField titulaireTF;
	/**
	 * JavaFX injecte l'objet de type TextField dans la variable 'soldeTF' l'élément
	 * TextField déclaré dans 'Main.java' doit posséder l'identifiant 'soldeTF' :
	 * fx:id="soldeTF"
	 */
	@FXML
	TextField soldeTF;
	 ...
	 * l'élément Button déclaré dans 'Main.java' doit posséder l'identifiant
	 * 'supprimerCompteBtn' : fx:id="supprimerCompteBtn"
	 */
	@FXML
	 ...
	 * l'élément Button déclaré dans 'Main.java' doit posséder l'identifiant
	 * 'modifierCompteBtn' : fx:id="modifierCompteBtn"
	 */
	@FXML
	Button modifierCompteBtn;

	/**
	 * eventhandler du button 'ajouterCompteBtn' l'élément Button déclaré dans
	 * 'Main.java' doit posséder l'attribut 'onAction=#ajouterCompte'
	 */
	@FXML
	public void ajouterCompte(ActionEvent event) throws IOException {
		...
		AjouterCompteController ajoutCompteController = (AjouterCompteController) loader.getController();
		// passer l'objet banque au controller
		ajoutCompteController.setBanque(banque);
	...
	public void supprimerCompte(ActionEvent event) {
		...
		if (!resultat.isEmpty()) {
			...
			if (buttonType == ButtonType.OK) {
				int selectedIndex = compteListe.getSelectionModel().getSelectedIndex();
	...
	public void modifierCompte(ActionEvent event) {
		System.out.println(event);
		Compte compte = compteListe.getSelectionModel().getSelectedItem();
...
//		compteListe.getItems().addAll("java", "Python", "C++");
		...
		compteListe.getSelectionModel().getSelectedItems().addListener(new ListSelectionListener());
		...
		BooleanBinding equalToBinding = Bindings.size(compteListe.getSelectionModel().getSelectedItems()).isEqualTo(0);
		supprimerCompteBtn.disableProperty().bind(equalToBinding);
		modifierCompteBtn.disableProperty().bind(equalToBinding);

		leftPane.maxWidthProperty().bind(splitPane.widthProperty().multiply(0.3));
		leftPane.minWidthProperty().bind(splitPane.widthProperty().multiply(0.3));
	}
